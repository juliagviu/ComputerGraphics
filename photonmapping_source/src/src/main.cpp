...
int main(int argc, char* argv[])
{
	...
	char *name_file = nullptr, *default_name_file = const_cast<char *>("ambient_6.ppm");
	name_file = default_name_file;

	unsigned int scene = 1;

	unsigned int photons_global = 1000000,//1000000,
				 photons_caustic = 1000000, //1000000,//000000,
				 photons_volume = 1500000,
				 max_shots = 4000000,
				 nb_nearest_photons = 30;

	// Ambient participating media
	//Foggy* foggy = new Foggy(0.2,0.3, Vector3(1.0,1.0,1.0));
	// Volumetric photon mapping
	Foggy* foggy = new Foggy(0.5, 0.8, 0.1);
	// No fog
	//Foggy* foggy = nullptr;
	...
	BSDF* phong = new Phong(w, Vector3(.085,.85,.085), 10);
	BSDF* green = new Lambertian(w, Vector3(.085,.85,.085));
    BSDF* orange = new Lambertian(w, Vector3(.85,.6,.02));


	Triangle* floor1 = new Triangle( Vector3(-1.5,0,3.1),Vector3(1.5,0.,3.1),
									 Vector3(-1.5,0.,-1.5), white);
	w->add_object(floor1);
	Triangle* floor2 = new Triangle(Vector3(-1.5,0,-1.5), Vector3(1.5,0.,3.1),
									Vector3(1.5,0.,-1.5), white);
	w->add_object(floor2);

	Triangle* ceil1 = new Triangle(Vector3(1.5,2.,3.1), Vector3(-1.5,2,3.1),
								   Vector3(-1.5,2.,-1.5), white);
	w->add_object(ceil1);
	Triangle* ceil2 = new Triangle(Vector3(1.5,2.,3.1), Vector3(-1.5,2,-1.5),
								   Vector3(1.5,2.,-1.5), white);
	w->add_object(ceil2);
	...
	Triangle* left1 = new Triangle(Vector3(-1,2.5,-1.5),Vector3(-1,2.5,3.1),
								   Vector3(-1,-0.5,-1.5), red);
	w->add_object(left1);
	Triangle* left2 = new Triangle(Vector3(-1,0,-1.5), Vector3(-1,2.5,3.1),
								   Vector3(-1,-.5,3.1), red);
	w->add_object(left2);

	Object3D* right1 = new Triangle(Vector3(1,2.5,3.1), Vector3(1,2.5,-1.5),
									Vector3(1,-.5,-1.5), green);
	w->add_object(right1);
	Object3D* right2 = new Triangle(Vector3(1,2.5,3.1), Vector3(1,-.5,-1.5),
									Vector3(1,-.5,3.1), green);
	w->add_object(right2);

	Triangle* front1 = new Triangle(Vector3(1.5,2.5,3.1), Vector3(-1.5,2.5,3.1),
									Vector3(-1.5,-.5,3.1), white);
	w->add_object(front1);
	Triangle* front2 = new Triangle(Vector3(1.5,2.5,3.1), Vector3(-1.5,-.5,3.1),
									Vector3(1.5,-.5,3.1), white);
	w->add_object(front2);

/*    Triangle* t1 = new Triangle(Vector3(1.5,2.5,2), Vector3(-1.5,2.5,2),
                                    Vector3(-1.5,-.5,2), white);
    Triangle* t2 = new Triangle(Vector3(1.5,2.5,2), Vector3(-1.5,-.5,2),
                                    Vector3(1.5,-.5,2), white);
    w->add_object(t1);
    w->add_object(t2);*/
	...
	case 1:
	{	
		Object3D* sphere1 = new Sphere(Vector3(0,1,0.5), 0.3, glass);
		//Object3D* sphere1 = new Sphere(Vector3(0.5,0.3,.5), 0.3, glassFresnel);
	...
	{	
        ...
        //Object3D* sphere1 = new Sphere(Vector3(0.5,0.3,.5), 0.3, phong);
		w->add_object(sphere1);

		Mesh* bunny = new Mesh("bunny.obj", red);
		w->add_object(bunny);
/*        Object3D* sphere1 = new Sphere(Vector3(0,0.8,0), 0.6, glass);
	...
	pm = new PhotonMapping(w, r, foggy, photons_global, photons_caustic, photons_volume, max_shots, nb_nearest_photons, false);
	engine = new RenderEngine(w, r, film, &camera, pm);
